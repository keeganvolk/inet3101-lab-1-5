...
int main(int argc, char *argv[]) {
	...
	while (1) {
		if (num_elements <= size) { // if the array is full, reallocate it adding space for another int
			array = realloc(array, (size + 1) * sizeof(*array));
		}
		scanf("%d", &array[size]); // add input to array, increment num_elements
		size++;
		printf("%d numbers in array: ", size);
